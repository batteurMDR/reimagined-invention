...
function listCells() {
    ...
    const movments = database_1.Database.getInstance().getRawMovments();
    for (const cell of cells) {
        ...
        for (let movment of movments) {
            if (movment.to === cell.id) {
                input += (movment.quantity / getFactorFromCategory(cell.category));
            }
            if (movment.from === cell.id) {
                output += (movment.quantity / getFactorFromCategory(cell.category));
            }
        }
...
function getFactorFromCategory(category) {
    if (category === '1.4G/S') {
        return 5;
    }
...
function getColorFromquantityAndMax(quantity, max) {
